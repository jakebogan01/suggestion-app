<script>
     ...
     let inputValues = {email: "", password: "", confirmPassword: ""};
     let isLoading = null;
     let error = null;
                    ...
                    <section class="mt-8">
                         <div class="mt-6">
                              ...
                              <form on:submit|preventDefault={handleSubmit}>
                                   <div>
                                        <label class="block font-medium text-sm text-white" for="name"><span>Username</span></label>
                                        <input
                                   ...
                                   <div class="mt-4">
                                        <label class="block font-medium text-sm text-white" for="password_confirmation"><span>Confirm Password</span></label>
                                        <input
                                                on:change={(e) => {inputValues.confirmPassword = e.target.value; error = null;}}
                                                bind:value={inputValues.confirmPassword}
                                                class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full"
                                   ...
                                   <div class="flex items-center justify-end mt-4">
                                        ...
                                        <button type="submit" class="ml-4 inline-flex items-center px-4 py-2 border {error ? 'bg-red-500 hover:bg-red-400 animate-[wiggle_0.2s_ease-in-out_0s_2_backwards]' : 'bg-[#238AB6] hover:bg-[#186489]'} border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray transition ease-in-out duration-150" disabled={isLoading || error}>Register</button>
                                   </div>
